...
int main(int argc, char * argv[])
{
    ...
    const NumaBroadcastContext numa_context = pio_create_numa_broadcast_context();
    ...
    for(int i = 0; i < num_vars; i++) {
        ...
        if(numa_context.numa_rank == 0){
            dataIn = reinterpret_cast<float*>(readFloatData(filename.c_str(), &nbEle));
            if (dataIn == NULL || nbEle == 0) {
                printf("ERROR! NUMA %d leader rank %d failed to read input file %s\n",
                       numa_context.numa_id, world_rank, filename.c_str());
        ...
        pio_numa_bcast_size(&nbEle, numa_context);
        if(numa_context.numa_rank != 0){
            dataIn = (float *) malloc(nbEle * sizeof(float));

            if (dataIn == NULL) {
                printf("ERROR! Failed to allocate input buffer on rank %d (NUMA %d)\n",
                       world_rank, numa_context.numa_id);
                MPI_Abort(MPI_COMM_WORLD, 1);
                return 1;
            }
        }
        pio_numa_bcast_floats(dataIn, nbEle, numa_context);
        ...
        MPI_Barrier(MPI_COMM_WORLD);
        if(world_rank == 0) start = MPI_Wtime();
        if(numa_context.numa_rank == 0) absErrorBound = eb * dataRange(dataIn, nbEle);
        pio_numa_bcast_double(&absErrorBound, numa_context);
